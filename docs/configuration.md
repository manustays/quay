...
| `agentIntervalSec` | number | `5` | How often (seconds) the agent radar rebuilds its session list, **while the popover is open**. The always-on tray waiting badge rides `pollIntervalSec` instead, so raising this never delays the menubar. A pass is now a directory read plus one `sysinfo` refresh — no `ps` fork and no 200 ms sampling stall — so there is far less to save by raising it than there used to be. Minimum 1. See [agent radar](agent-radar.md). |
